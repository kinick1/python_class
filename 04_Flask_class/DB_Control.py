...
def create_tbl(conn,cur):
    ...
    try: 
        # 쿼리문을 DB로 전송해서 실행
        # 커서를 통해서 실행성공 유무를 반환함(실패시 -1을 반환)
        cur.execute(query)
        
        # DB 실행결과를 DB에 반영하는 내용
        conn.commit()
        print('테이블 생성 완료')
    except cx_Oracle.DatabaseError as e:
        print("Error : ",e)
...
def insert_tbl(conn,cur,code, name,age):
    ...
    # 딕셔너리의 키값을 쿼리문에 설정해 쿼리문을 생성
    query="insert into flask_table values(:code,:name,:age)"
    
    
    try: 
        # 딕셔너리 값을 query문에 할당해서 DB로 전송 하고 실행
...
# 테이블에서 조건에 맞는 code를 기준으로 데이터만 검색하는 함수
...
# 삭제하는 함수
def del_tbl(conn,cur,code):
    # (1)파라미터로 받은 데이터를 딕셔너리로 만든다.
    input_data={"code":code}
    
    # (2)쿼리문을 정의 - 콜론:변수명 형태로 딕셔너리의 키 값 위치를 설정
    query= "delete from flask_table where code=:code"
    
    # (3) 예외처리
    try :
        # (4) exectue()로 쿼리를 전송해 실행 딕셔너리 할당
        cur.execute(query,input_data)
